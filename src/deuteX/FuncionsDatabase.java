	...
	public static boolean existeixUsuari(Connection conn, String usuari){
		...
		if(conn != null)
	    {
			try {
				...
				while(rs.next())
					//Comprova si l'usuari estÃ  a la Base de Dades
				if(usuari.equals(rs.getString("nom"))){
					//Retorna un valor cert perquÃ¨ pugui continuar.
					return true;
				}
		...
		//Retorna un valor fals perquÃ¨ l'usuari no existeix.
		return false;
	}
	...
	public static boolean validarLogin(Connection conn, String usuari, String password){
		...
		if(conn != null)
	    {
			try {
				...
				while(rs.next())
				//Si el nom i la contrasenya sÃ³n correctes.
